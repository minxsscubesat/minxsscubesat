...
;
; USAGE
;   is1_daxss_read_packets, input, sci=sci, log=log, dump=dump, /hexdump, /verbose
;
; INPUTS:
...
;
;+
pro is1_daxss_read_packets, input, sci=sci, log=log, dump=dump, hexdump=hexdump, fm=fm, $
  ...
  if (n_params() lt 1) then begin
    print, 'USAGE: is1_daxss_read_packets, input_file, sci=sci, log=log, dump=dump, $'
    print, '              				fm=fm, /hexdump, /verbose, /debug'
    input=' '
  endif
  IF size(input[0], /TYPE) EQ 7 THEN if (strlen(input) lt 2) then begin
    ; find file to read
    input = dialog_pickfile(/read, title='Select IS-1 DAXSS dump file to read', filter='ccsds_*')
