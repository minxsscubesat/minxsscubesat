...
;
;	IDL> ; restore the daxss_l0c_merged_YYYY_DOY.sav file
...
PRO daxss_make_level0c, telemetryFileNamesArray = telemetryFileNamesArray, yyyydoy = yyyydoy, $
						            yyyymmdd = yyyymmdd, use_csv_file=use_csv_file, $
  ...
  ; 3. Write DAXSS data structures to disk as IDL save file
  ;
  ;if (merged ne 0) then fileBase = 'daxss_l0c_merged_' else fileBase = 'daxss_l0c_'
  ...
  ; if keyword_set(verbose) then stop, 'DEBUG at end of daxss_make_level0c...'
END
