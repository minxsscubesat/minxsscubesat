...
;   3. Write DAXSS Level 0a binary file (to be read by is1_daxss_beacon_read_packets.pro)
;
;
; EXAMPLE USAGE
; IDL> daxss_make_level0a, /verbose
;
...
;
;+
PRO daxss_make_level0a, VERBOSE = VERBOSE, DEBUG=DEBUG
...
;
;   3. Write DAXSS Level 0a binary file
;
if (bdata_length gt 0) then begin
  flightModelString = 'fm4'
  out_path = getenv('minxss_data')+path_sep()+flightModelString+path_sep()+'csv_files'+path_sep()
  out_file = 'daxss_l0a_csv_merged.bin'
  fullFilename = out_path + out_file

  IF keyword_set(verbose) THEN message, /INFO, 'Saving ' + strtrim(bdata_length,2) + $
      ' bytes of data for DAXSS Level 0a packets into ' + fullFilename
  ...
  if keyword_set(verbose) then message, /INFO, 'ERROR finding any IS-1 / DAXSS data to save for Level 0a.'
endelse

if keyword_set(debdaug) then stop, 'DEBUG at end of daxss_make_level0a...'
;  clear memory
bdata = 0L
