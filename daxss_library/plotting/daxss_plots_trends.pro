;+
; NAME:
;   daxss_plots_trends.pro
;
; PURPOSE:
;   Plot trends of Telemetry (Tlm) data and merge into single PDF file.
;   User can pass DAXSS Tlm packets or specify date/time range for the plots.
...
;   tlmType [string]:         The type of data to plot e.g., 'SCI', 'LOG', or 'DUMP'. Default is SCI.
...
;   3. Make plots
;+
PRO daxss_plots_trends, packet, timeRange = timeRange, items = items, tlmType = tlmType, level = level, layout = layout, $
  ...
  IF keyword_set(MISSION_LENGTH) THEN BEGIN
    timerange = [20190513, JPMjd2yyyymmdd(systime(/JULIAN, /UTC))]
  ENDIF
  ...
  IF (readFiles NE 0) THEN BEGIN
    ...
    IF (fcount GT 0) THEN BEGIN
      ...
      pdf_type = 'sci'
    ENDIF ELSE BEGIN
      message, /INFO, ' could not find file = ' + data_file
      return
    ENDELSE
  ENDIF
  ...
  IF keyword_set(MISSION_LENGTH) THEN BEGIN
    ...
    FOR yyyyDoy = time1, time2 DO BEGIN
      ...
      IF fcount NE 0 THEN BEGIN
        ...
        sciTemp = [sciTemp, sci]
      ENDIF ELSE message, /INFO, ' could not find file: ' + data_file
    ENDFOR

    ; Move the concatenated data from sciTemp to sci
  ...
  ; 3. Make plots
  ;
  ; Define indices into sci packet structure FOR each page AND THEN loop to make plots FOR each page
  ...
  FOR k=0L,page_count-1 DO BEGIN
    ...
    IF keyword_set(pdf) THEN BEGIN
      ...
      IF errorStatus NE 0 THEN BEGIN
        message, /INFO, 'Ran out of plots (skipped time plots and any other arrays). Closing PDF and exiting loop.'
        plotobj[0] = plot(/TEST, TITLE = 'IGNORE THIS PLOT. IT IS JUNK', /BUFFER)
        plotobj[0].close
        catch, /CANCEL
        BREAK
      ENDIF
