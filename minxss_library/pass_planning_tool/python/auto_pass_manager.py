...
def main(script): #This is what calls the minxss_monitor_pass_times code and sets it all up
    ...
    if p.cfg.use_satpc == 1:
        p.SATPC32_monitor() #calling this twice because it creates a lot of junk in the console on startup
...
class pass_manager:
    def __init__(self):
        ...
        if(test_pass_conflicts_enabled == 1):
            self.tester = pass_manager_tests()
        ...
        for satellite_ini_file in self.cfg.sat_ini_files:
            ...
            self.sat_pass_managers[tmp_sat_cfg.sat_name] = satellite_pass_manager(tmp_sat_cfg, self.sat_emails[tmp_sat_cfg.sat_name], self.cfg)
            #make sure any instance of hydra that may have been left open is now closed
            self.sat_pass_managers[tmp_sat_cfg.sat_name].KillHydra()

        #SATPC variables
        self.tle_contents = None
        if self.cfg.use_satpc == 1:
            self.satpc32_exe = exe_management(self.cfg.satpc_dir, self.cfg.satpc_exe_name,1)
            self.satpc32_server_exe = exe_management(self.cfg.satpc_dir, self.cfg.satpc_server_exe_name,0)
...
class satellite_pass_manager():
    ...
    def __init__(self, cfg, email_module, global_cfg):
        ...
        self.isis_exe = exe_management(self.cfg.isis_dir,self.cfg.isis_exe_name,1)
    ...
    def run_pass(self,info,is_quick_exit):
        ...
        self.SleepUntilPassIsDone(info)
        ...
        self.PassAnalysis(info)

        print("\r\n**********************  Done with {0} pass! **********************\r\n\r\n".format(info.sat_name))

    def SleepUntilPassIsDone(self, info):
        while(1):
