...
oplot, xpeak*[1,1], 10.^!y.crange, line=2
...
temp = min(abs(sphour - xpeak), wpeak )
if (temp gt limit1) then begin
	print, 'ERROR finding pre-flare spectrum'
	if keyword_set(debug) then stop, 'DEBUG...'
	return
endif
...
sp_peak = reform(sp1day[*,wpeak])
count1=1.
;  attempt to average two adjacent data points
if wpeak gt 0 then begin
  if abs(sphour[wpeak-1]-sphour[wpeak]) lt limit2 then begin
    sp_peak += reform(sp1day[*,wpeak-1])
    count1 += 1.
  endif
endif
if wpeak lt (num_sp1-1) then begin
  if abs(sphour[wpeak+1]-sphour[wpeak]) lt limit2 then begin
    sp_peak += reform(sp1day[*,wpeak+1])
    count1 += 1.
  endif
...
;
  ...
  if (num_xrs2 gt 1) then begin
  	...
  	if (ftemp gt 1E-3) then begin
  	  flare_str = 'X' + string(long(ftemp/1E-4),format='(I2)')
  	endif else if (ftemp gt 1E-4) then begin
  	  flare_str = 'X' + string(ftemp/1E-4,format='(F3.1)')
  	endif else if (ftemp gt 1E-5) then begin
  	  flare_str = 'M' + string(ftemp/1E-5,format='(F3.1)')
  	endif else if (ftemp gt 1E-6) then begin
  	  flare_str = 'C' + string(ftemp/1E-6,format='(F3.1)')
  	endif else if (ftemp gt 1E-7) then begin
  ...
  plot, esp, sp_peak, psym=10, /nodata, xr=erange, xs=1, yr=yrange2, ys=1, /xlog, /ylog, $
	xtitle='Energy (keV)', ytitle=ytitle2, title=mtitle2
  oplot, esp, sp_pre, psym=10, color=cc[3]
  oplot, esp, sp_peak, psym=10
...
if not keyword_set(raw_counts) then begin
  ...
  if keyword_set(oplot2013) then begin
  	...
  	xyouts, xx, yy*my^2, '2013', color=cc[0]
  endif
