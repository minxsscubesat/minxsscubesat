...
; ADCS mode
p1 = plot(adcs2.time_jd, adcs2.ADCS_MODE, title = 'ADCS mode in hk', '*--', font_size = 14, xtickunits = dates, margin = margin, $
          yrange = [-1, 2], ytickname = ['','','Sun Point', '', 'Fine Ref', '', ''], thick = 2)
p2 = plot(adcs3.time_jd, adcs3.adcs_info and '01'x, title = 'ADCS mode in page 3', '*--', font_size = 14, xtickunits = dates, margin = margin, $
          yrange = [-1, 2], ytickname = ['','','Sun Point', '', 'Fine Ref', '', ''], thick = 2)
p3 = plot(adcs4.time_jd, adcs4.adcs_info and '01'x, title = 'ADCS mode in page 4', '*--', font_size = 14, xtickunits = dates, margin = margin, $
          yrange = [-1, 2], ytickname = ['','','Sun Point', '', 'Fine Ref', '', ''], thick = 2)
...
; Battery voltage
p1 = plot(hk.time_jd, hk.eps_fg_volt, title = 'Battery voltage [V]', symbol = '*', linestyle = 'none', font_size = 14, xtickunits = dates, margin = margin)
...
; Temperatures
p2 = plot(hk.time_jd, hk.eps_batt_temp1, 'tomato', symbol = '*', linestyle = 'none', title = 'Battery temperature [ºC]', font_size = 14, xtickunits = dates, margin = margin)
p3 = plot(hk.time_jd, hk.eps_batt_temp2, 'dodger blue', symbol = '*', linestyle = 'none', yrange = [-10, 20], /overplot)
p4 = plot(hk.time_jd, hk.cdh_temp, 'tomato', symbol = '*', linestyle = 'none', title = 'CDH Temperature [ºC]', font_size = 14, xtickunits = dates, margin = margin)
...
; Wheel speed
p1 = plot(hk.time_jd, hk.XACT_WHEEL1MEASSPEED * rad2rpm, symbol = '*', linestyle = 'none', 'tomato', title = 'Wheel speeds [RPM]', font_size = 14, name = '1',  xtickunits = dates, margin = margin)
...
p1.yrange = [-6000, 6000]
p = plot(p1.xrange, [0, 0], '--', /OVERPLOT)
p1.save, saveloc + 'wheel speeds.png'

; IMU rates
mins = (adcs4.time_jd - adcs4[0].time_jd) * 24. * 60.
p1 = plot(adcs4.time_jd, adcs4.IMU_BODY_RATE1 * !RADEG, symbol = '*', linestyle = 'none', 'tomato', font_size = 14, title = 'IMU body rates [º s$^{-1}$]', xtickunits = dates, margin = margin)
p2 = plot(adcs4.time_jd, adcs4.IMU_BODY_RATE2 * !RADEG, symbol = '*', linestyle = 'none', 'lime green', /overplot)
p3 = plot(adcs4.time_jd, adcs4.IMU_BODY_RATE3 * !RADEG, symbol = '*', linestyle = 'none', 'dodger blue', /overplot)
p1.save, saveloc + 'imu.png'

; Torque rod duty cycle
p1 = plot(adcs4.time_jd, adcs4.tr2_duty_cycle, '2', COLOR = 'lime green', symbol = '*', linestyle = 'none', FONT_SIZE = 14, TITLE = 'Torque rod duty cycle [%]', xtickunits = dates, margin = margin)
p2 = plot(adcs4.time_jd, adcs4.tr3_duty_cycle, '2', COLOR = 'dodger blue', symbol = '*', linestyle = 'none', /OVERPLOT)
mins3 = (adcs3.time_jd - adcs3[0].time_jd) * 24. * 60.
p3 = plot(adcs3.time_jd, adcs3.tr1_duty_cycle, '2', COLOR = 'tomato', symbol = '*', linestyle = 'none', /current, margin = margin, xshow = 0, yshow = 0, axis_style = 4, xrange = p1.xrange, yrange = p1.yrange)
p1.save, saveloc + 'torque rod duty cycle.png'

; Sun body vector
p1 = plot(adcs4.time_jd, adcs4.SUNBODY_X, symbol = '*', yrange = [-1.5, 1.5], linestyle = 'none', 'tomato', font_size = 14, title = 'Sun body X', xtickunits = dates, margin = margin)
p1.save, saveloc + 'sunbodyx.png'

; Momentum body vector
mins = (adcs3.time_jd - adcs4[0].time_jd) * 24. * 60.
p1 = plot(adcs4.time_jd, adcs3.BODY_ONLY_MOMENTUM_IN_BODY1, symbol = '*', linestyle = 'none', 'tomato', font_size = 14, title = 'Momentum Body Only [Nms]', xtickunits = dates, margin = margin)
p2 = plot(adcs4.time_jd, adcs3.BODY_ONLY_MOMENTUM_IN_BODY2, symbol = '*', linestyle = 'none', 'lime green', /overplot)
p3 = plot(adcs4.time_jd, adcs3.BODY_ONLY_MOMENTUM_IN_BODY3, symbol = '*', linestyle = 'none', 'dodger blue', /overplot)
...
; System momentum
p1 = plot(adcs3.time_jd, totalMomentum, '2', COLOR = 'goldenrod', FONT_SIZE = 14, $
          TITLE = 'Total System Momentum [Nms]', $
          xtickunits = dates, margin = margin)
p2 = plot(p1.xrange, [0.011, 0.011], '2--', COLOR = 'tomato', /OVERPLOT)
t2 = text(0.8, 0.8, 'wheel cutoff', ALIGNMENT = 1, COLOR = 'tomato', FONT_SIZE = 12)
