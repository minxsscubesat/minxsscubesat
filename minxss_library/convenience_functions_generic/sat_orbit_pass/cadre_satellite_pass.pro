...
;
;	OUTPUT
;		IDL save set of passes in cadre_passes_latest.sav
;		IDL save set of passes in cadre_passes_DATE_TIME.sav
;		HTML table of passes in cadre_passes_latest.html
;		HTML table of passes in cadre_passes_DATE_TIME.html
;
;		pass_data		Optional return of "passes" data structure array
;		pass_conflict	Optional return of CADRE pass conflict information
;
;	PROCEDURE
;	1.  Download latest TLE for CADRE
...
;
pro cadre_satellite_pass, date_range, debug=debug, verbose=verbose, $
...
if keyword_set(verbose) then print, 'CADRE_SATELLITE_PASS is getting passes for ', date_str
...
;
;  *****  DEFINE CADRE MISSION NAME FOR TLE SEARCH
;		For now, use ISS
;
mission = 'ISS (ZARYA)'
satid = 25544L		; Satellite ID for ISS is 25544
mission = 'CADRE'
satid = 41475L ; or it could be 41474L  (MinXSS-1 ?)

gs_long_lat = [ -83.7409D0, 42.2761D0  ]  ; CADRE (U. Michigan) longitude and latitude
...
if not keyword_set(debug) then begin
	...
	tle_download_latest, tle, satid=satid, path=path_name, /output, verbose=verbose
endif
...
if not keyword_set(no_conflict) then begin
	...
	cadre_passes = passes
	cadre_pass_conflict = pass_conflict
	...
	minxss_pass_conflict = pass_conflict
	passes = cadre_passes
	pass_conflict = cadre_pass_conflict
	;
	;  check each MinXSS pass for CADRE pass conflict
	;		MinXSS pass has to have more than 2 minutes overlap and
	;		MinXSS has to have larger maximum elevation over its ground station (GS) than CADRE GS
	;
	...
	if keyword_set(debug) or keyword_set(verbose) then begin
		print, 'Number of CADRE Passes lost to MinXSS = ', strtrim(lost_count, 2), ' (', $
...
save_name1 = 'cadre_passes_latest.sav'
save_name2 = 'cadre_passes_' + date_str + '.sav'
save_name3 = 'cadre_location_latest.sav'
...
html_name1 = 'cadre_passes_latest.html'
html_name2 = 'cadre_passes_' + date_str + '.html'
...
printf, lun, '<html>'
printf, lun, '<head>'
printf, lun, '<title>CADRE CubeSat Passes for ' + date_str + '</title>'
printf, lun, '<style>'
printf, lun, 'table {'
...
printf, lun, '<body>'
printf, lun, '<table>'
printf, lun, '<caption>CADRE CubeSat Passes for ' + date_str + '</caption>'
printf, lun, '<thead>'
...
printf, lun, '</table>'
printf, lun, '<P>'
printf, lun, 'This table was generated by cadre_satellite_pass.pro on ' + systime(/utc)
printf, lun, '<P>'
printf, lun, '</body>'
printf, lun, '<html>'
close, lun
...
csv_name1 = 'cadre_passes_latest.csv'
csv_name2 = 'cadre_passes_' + date_str + '.csv'

if keyword_set(verbose) then print, 'Saving "passes" table to ', csv_path+csv_name1
openw, lun, csv_path+csv_name1, /get_lun
printf, lun, 'CADRE CubeSat Passes for ' + date_str
...
if keyword_set(debug) then stop, 'DEBUG cadre_satellite_pass() results ...'
