...
;
; USAGE
;   minxss_read_packets, input, hk=hk, sci=sci, log=log, diag=diag, xactimage=xactimage, $
;                        adcs1=adcs1, adcs2=adcs2, adcs3=adcs3, adcs4=adcs4, /hexdump, /verbose
;
; INPUTS:
...
;   xcatimage [structure]:    Return array of XACT Star Tracker image packets
;
; OPTIONAL OUTPUTS:
;   None
;
...
;   2014-08-20: Chris Moore:      HK and SCI updated for TLM handbook revision D.25
;   2014-08-24: Tom Woods:        Updated SCI to handle X123 compression
;   2014-10-08: James Paul Mason: Updated to ignore CDI packet headers, which don't show up from ISIS socket output, and
;                                 Beth will soon be updating ISIS to not include CDI in the telemetry .out files
;   2014-10-13: Chris Moore:      HK and SCI updated for TLM handbook revision D.28
;   2014-10-14: James Paul Mason: Can now handle real time data passed as a bytarr or work the standard way.
;                                 Now adds two sync bytes to the beginning so that the first packet is not lost.
;   2014-10-17: James Paul Mason: Found error in SPS interpretation. Last byte was being added by 4 instead of multiplied by 4 like the other bytes
;                                 Returns -1 in sci output if in bytarr input mode and input does not contain complete sci packet
;   2015-01-15: Chris Moore:      Added structure and functionality to read Star Tracker Images
;   2015-01-26: James Paul Mason: Reformatted header.
;   2015-01-31: Tom Woods:        Added playback IDs and four different ADCS packets (but incomplete read of them)
;                                 and made consistent structure defines (APID, SEQ_FLAG, SEQ_COUNT, DATA_LENGTH, TIME)
;   2015-03-06: Tom Woods:        Version 34: Added ADCS data extraction
;   2015-03-06: Chris Moore:      Version 35: Added ADCS extras
;   2015-03-09: Tom Woods:        Version 36: Merged Version 34 & 35
...
;   2015-08-26: Tom Woods:        Version 37: Merged ADCS 1-4 packet decomposition
;   2015-09-01: James Paul Mason: Version 38: Added keyword EXPORT_RAW_ADCS_TLM to dump raw bytes for BCT.
...
;   2016-06-11: Amir Caspi:       Fix partial packets (gaps) for SCI packets
...
pro minxss_read_packets, input, hk=hk, sci=sci, log=log, diag=diag, xactimage=xactimage, $
  ...
  adcs1_struct1 =  {  apid: 0, seq_flag: 0, seq_count: 0, data_length: 0L, time: 0.0D0, $
    ...
    tai_seconds:0.0, cycle_time:0.0, julian_date_tai:0UL, time_valid:0B, orbit_time:0.0, $
    q_ecef_wrt_eci1:0.0, q_ecef_wrt_eci2:0.0, q_ecef_wrt_eci3:0.0, q_ecef_wrt_eci4:0.0, $
    orbit_position_eci1:0.0, orbit_position_eci2:0.0, orbit_position_eci3:0.0, $
    orbit_position_ecef1:0.0, orbit_position_ecef2:0.0, orbit_position_ecef3:0.0, $
    orbit_velocity_eci1:0.0,  orbit_velocity_eci2:0.0,  orbit_velocity_eci3:0.0, $
    orbit_velocity_ecef1:0.0, orbit_velocity_ecef2:0.0, orbit_velocity_ecef3:0.0, $
    ...
    attitude_quaternion1:0.0, attitude_quaternion2:0.0, attitude_quaternion3:0.0, attitude_quaternion4:0.0, $
    attitude_filter_residual1:0.0}
  ...
  adcs2_struct1 =  {  apid: 0, seq_flag: 0, seq_count: 0, data_length: 0L, time: 0.0D0,  $
    cdh_info: 0B, adcs_info: 0B, adcs_group: 0U, SpareByte: 0, checkbytes: 0U, SyncWord: 0U, $
     attitude_filter_residual2: 0.0, attitude_filter_residual3: 0.0, $
    body_frame_rate1: 0.0, body_frame_rate2: 0.0, body_frame_rate3: 0.0, $
    ...
    commanded_attitude_quat1: 0.0, commanded_attitude_quat2: 0.0, commanded_attitude_quat3: 0.0, commanded_attitude_quat4: 0.0, $
    commanded_rate1: 0.0, commanded_rate2: 0.0, commanded_rate3: 0.0, $
    commanded_accel1: 0.0, commanded_accel2: 0.0, commanded_accel3: 0.0, $
  ...
  adcs3_struct1 =  {  apid: 0, seq_flag: 0, seq_count: 0, data_length: 0L, time: 0.0D0,  $
    ...
    tracker_covariance_matrix7: 0.0, tracker_covariance_matrix8: 0.0, tracker_covariance_matrix9: 0.0, $
    tracker_max_residual: 0B, tracker_max_residual_first_pass: 0B, tracker_analog_gain: 0.0, $
    tracker_magnitude_correction: 0.0, detector_temp_command: 0.0, $
    tracker_bright_magnitude_limit: 0.0, tracker_dim_magnitude_limit: 0.0, tracker_tec_duty_cycle: 0.0, $
    tracker_peak_3sigma_noise: 0.0, tracker_peak_mean_background: 0.0, tracker_median_3sigma_noise: 0.0, $
    tracker_median_mean_background: 0.0, tracker_operating_mode: 0B, $
...
;          if (sciPacketIncomplete and (sci_count eq 0)) then sci = -1
        ...
        if arg_present(adcs1) AND n_elements(data) GT pindex+219 THEN BEGIN
...
; 8 Dec 2018: Amir Caspi: FM1 ADCS packet is 220 bytes! 6 bytes of zeros before CRC/Sync... moving indices to accommodate
          ...
          adcs1_struct1.tai_seconds = (ishft(ulong(data[pindex+68]),24) + ishft(ulong(data[pindex+69]),16) $
...
; 8 Dec 2018: Amir Caspi + Tom Woods: EXCEPT... turns out all the extra bytes ARE at the end, because the data is byte-copied into the union...
          ...
          ;  adcs1_struct1.adcs_data[i] = ((data[pindex+16+i]))  ;see adcs1_data Notes below
          ; ENDFOR
      ;
          ;adcs1_data Notes:
        ...
        ;  ************************
        ;
        if arg_present(adcs2) AND n_elements(data) GT (pindex + 219) then begin
...
; 8 Dec 2018: Amir Caspi + Tom Woods: EXCEPT... turns out all the extra bytes ARE at the end, because the data is byte-copied into the union...
        ...
        ;  ************************
        ;
        if arg_present(adcs3) AND n_elements(data) GT (pindex + 219) then begin
...
; 8 Dec 2018: Amir Caspi + Tom Woods: EXCEPT... turns out all the extra bytes ARE at the end, because the data is byte-copied into the union...
        ...
        ;  ************************
        ;
        if arg_present(adcs4) AND n_elements(data) GT (pindex + 219) then begin
  ...
  IF keyword_set(EXPORT_RAW_ADCS_TLM) THEN BEGIN
    IF typename(input) EQ 'STRING' THEN BEGIN
      IF adcs1Raw NE !NULL OR adcs2Raw NE !NULL OR adcs3Raw NE !NULL OR adcs4Raw NE !NULL THEN BEGIN
        ...
        IF fm NE 0 THEN file_copy, input, getenv('minxss_data') + '/fm' + fmString + '/xact_tlm_exported/' + inputStringParsed.Filename, /OVERWRITE
      ENDIF
    ENDIF
