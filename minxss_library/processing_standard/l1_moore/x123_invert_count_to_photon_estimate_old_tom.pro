...
;
;
; CALLING SEQUENCE: result = X123_invert_count_to_photon_estimate(x123_energy_bins_kev,
;		converted_energy_bins_offset_bins, x123_measured_counts, $
;		counts_uncertainty= uncertainty_x123_measured_counts, $
;		minxss_instrument_structure_data_file= minxss_instrument_structure_data_file, $
...
;
; INPUTS:
;	x123 measured count spectrum (counts/s/kev), x123 energy bins (keV)
;
; INPUT KEYWORD PARAMETERS:
;	/use_detector_area, multiplies the input photon flux by the XP geometric aperture area,
;
;	can output uncertainties and contributions form different processes that contribute to the minxss spectrum
;
; RETURNS:
;	estimate of the input photon flux (photons/s/keV/cm^2)
;
;
; Details:
;
...
;minxss_instrument_structure_data_file: [INPUT] Full path and file name of the
;												minxss_instrument_structure_data_file (ARRAY)
;use_detector_area                  [BOOLEAN KEYWORD] set this keyword to use the detector area in the calculation
;
;uncertainty_X123_photon_estimate_ARRAY:  [OUTPUT/RETURNED KEYWORD]  returns  uncertainty of photon flux
;X123_photon_estimate_be_si_photopeak_only_ARRAY: [OUTPUT/RETURNED KEYWORD] estimate of input photon
;		flux derived form the be_si_photopeak contribution (ARRAY)
;uncertainty_X123_photon_estimate_be_si_photopeak_only_ARRAY: [OUTPUT/RETURNED KEYWORD] uncertainty
;		in the estimate of input photon flux derived form the be_si_photopeak contribution (ARRAY)
;X123_inverted_counts_to_photons_be_photoelectron_only_ARRAY:  [OUTPUT/RETURNED KEYWORD] estimate of
;		input photon flux derived form the be_photoelectron contribution (ARRAY)
;uncertainty_X123_inverted_counts_to_photons_be_photoelectron_only_ARRAY: [OUTPUT/RETURNED KEYWORD]
;		uncertainty in the estimate of input photon flux derived form the be_photoelectron contribution (ARRAY)
;X123_corrected_count_be_photoelectron_si_escape_ARRAY: OUTPUT/RETURNED KEYWORD] corrected count
;		contribution from be_photoelectrons (ARRAY)
;X123_count_contribution_Be_Si_photopeak_ARRAY:  [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from be_si_photopeak (ARRAY)
;X123_count_contribution_Be_photoelectron_ARRAY: [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from be_photoelectrons (ARRAY)
;X123_count_contribution_Si_escape_all_ARRAY:    [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from all si escape processes (ARRAY)
;X123_corrected_count_contribution_Si_escape_all_ARRAY: [OUTPUT/RETURNED KEYWORD] corrected count
;		contribution from be_si_escape all processes (ARRAY)
;X123_count_contribution_Si_k_escape_ARRAY:      [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from si k escape process (ARRAY)
;X123_corrected_count_contribution_Si_k_escape_ARRAY:  [OUTPUT/RETURNED KEYWORD] corrected count
;		contribution from silicon k escape process (ARRAY)
;X123_count_contribution_Si_l_2s_escape_ARRAY:   [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from si l 2s escape process (ARRAY)
;X123_corrected_count_contribution_Si_l_2s_escape_ARRAY: [OUTPUT/RETURNED KEYWORD] corrected count
;		contribution from si l 2s escape process (ARRAY)
;X123_count_contribution_Si_l_2p_escape_ARRAY:    [OUTPUT/RETURNED KEYWORD] measured count contribution
;		from si l 2p escape process (ARRAY)
;X123_corrected_count_contribution_Si_l_2p_escape_ARRAY:  [OUTPUT/RETURNED KEYWORD] corrected count
;		contribution from si l 2p escape process (ARRAY)
;X123_inverted_count_to_photon_estimate_valid_flag_ARRAY: [OUTPUT/RETURNED KEYWORD] flag indicating
;		the agreement between the returned, be_si_photopeak_only and be_photoelectron_only photon
;		flux estimation (ARRAY), flag = 1 if all three processes agree to within 10%
;		(this is the default value, which can be changed by setting the
;		X123_inverted_valid_flag_fractional_difference_level keyword to another FRACTIONAL value,
;		flag = 0 if the above is NOT true (or if there are no measured x123 counts at that energy
;X123_inverted_valid_flag_fractional_difference_level: [OUTPUT/RETURNED KEYWORD] sets a diferent
;		FRACTIONAL agreement value between the three inverted photon estimates, between the returned,
;		be_si_photopeak_only and be_photoelectron_only photon flux estimation
;
; REFERENCES:
;
...
function X123_invert_count_to_photon_estimate, x123_energy_bins_kev, converted_energy_bins_offset_bins, $
			x123_measured_counts, counts_uncertainty=uncertainty_x123_measured_counts, $
			minxss_instrument_structure_data_file=minxss_instrument_structure_data_file, $
			use_detector_area=use_detector_area, $
  			uncertainty_X123_photon_estimate_ARRAY=output_model_uncertainty_X123_photon_estimate, $
	X123_photon_estimate_be_si_photopeak_only_ARRAY= output_model_x123_inverted_counts_to_photons_be_si_photopeak_only, $
