...
catalog_struct = {timestamp:'', flare_class:'', number_data_points:0L}
...
FOR flareIndex = 0, n_elements(flare_start_jd) - 1 DO BEGIN
  ...
  write_csv, saveloc + 'CSV/' + jpmjd2iso(goesEvents[flareIndex].eventPeakTimeJd) + ' ' + strtrim(goesEvents[flareIndex].st$class, 2) + '.csv', minxss_flare, header = ['timestamp', strtrim(energy[good_energy_indices], 2)]
  save, minxss_flare, filename = saveloc + 'IDL Savesets/' + jpmjd2iso(goesEvents[flareIndex].eventPeakTimeJd) + ' ' + strtrim(goesEvents[flareIndex].st$class, 2) + '.sav'
  
  ; Add to catalog listing
  catalog_struct.timestamp = jpmjd2iso(goesEvents[flareIndex].eventPeakTimeJd)
  catalog_struct.flare_class = strtrim(goesEvents[flareIndex].st$class, 2)
  catalog_struct.number_data_points = npoints
...
write_csv, saveloc + 'CSV/MinXSS Flare Catalog.csv', catalog, header = ['Timestamp (flare peak)', 'Flare Class', 'Number MinXSS Datapoints']
