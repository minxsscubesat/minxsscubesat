;+
; NAME:
;   minxss_telemetry_interpolate.pro
;
; PURPOSE:
...
;
; CALLING SEQUENCE:
;   Example: outputY = minxss_telemetry_interpolate(adcs2.tag_name, adcs2.time, packetTimeStampsSpaceCraftTime, timeThresholdSeconds = 300)
;
; INPUTS:
...
;
; EXAMPLE: 
;   packetInterpolated = minxss_telemetry_interpolate(getenv('minxss_data') + '/fm1/level0d/hkTemporary.sav', sci.time, 60.)
;
; PROCEDURE:
...
;+
FUNCTION minxss_telemetry_interpolate, packetToProcessFilename, timeToInterpolateTo, timeThresholdSeconds, fm = fm, NO_SAVE = NO_SAVE
...
IF n_params() LT 3 THEN BEGIN
  print, 'USAGE: outputY = minxss_telemetry_interpolate(inputY, inputTime, timeToInterpolateTo, timeThresholdSeconds = timeThresholdSeconds'
  return, -1
ENDIF
...
IF strmatch(packetToProcessFilename, '*adcs1Temporary.sav') THEN packetToProcess = adcs1
IF strmatch(packetToProcessFilename, '*adcs2Temporary.sav') THEN packetToProcess = adcs2
IF strmatch(packetToProcessFilename, '*adcs3Temporary.sav') THEN packetToProcess = adcs3
IF strmatch(packetToProcessFilename, '*adcs4Temporary.sav') THEN packetToProcess = adcs4
...
IF ~keyword_set(NO_SAVE) THEN BEGIN
  IF strmatch(packetToProcessFilename, '*adcs1Temporary.sav') THEN BEGIN
    adcs1Interpolated = temporary(packetInterpolated)
    save, adcs1Interpolated, FILENAME = getenv('minxss_data') + '/fm' + strtrim(fm, 2) + '/level0d/adcs1InterpolatedTemporary.sav'
    return, adcs1Interpolated
  ENDIF
  IF strmatch(packetToProcessFilename, '*adcs2Temporary.sav') THEN BEGIN
    adcs2Interpolated = temporary(packetInterpolated)
    save, adcs2Interpolated, FILENAME = getenv('minxss_data') + '/fm' + strtrim(fm, 2) + '/level0d/adcs2InterpolatedTemporary.sav'
    return, adcs2Interpolated
  ENDIF
  IF strmatch(packetToProcessFilename, '*adcs3Temporary.sav') THEN BEGIN
    adcs3Interpolated = temporary(packetInterpolated)
    save, adcs3Interpolated, FILENAME = getenv('minxss_data') + '/fm' + strtrim(fm, 2) + '/level0d/adcs3InterpolatedTemporary.sav'
    return, adcs3Interpolated
  ENDIF
  IF strmatch(packetToProcessFilename, '*adcs4Temporary.sav') THEN BEGIN
    adcs4Interpolated = temporary(packetInterpolated)
    save, adcs4Interpolated, FILENAME = getenv('minxss_data') + '/fm' + strtrim(fm, 2) + '/level0d/adcs4InterpolatedTemporary.sav'
    return, adcs4Interpolated
  ENDIF
