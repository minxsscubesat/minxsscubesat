...
pro minxss_fit_2temperature, energy, flux, fit_flux, parameters=parameters, chi=chi, $
						noplot=noplot, eps=eps, debug=debug, integration_period=integration_period
...
if (parameters.logT ge 5.8) and (e_max ge e_max_abund) then begin
	...
	if (flux_ref gt (FLUX_MIN*6)) then begin
		...
		if (use_norm_save eq 0) then norm_cor = flux_ref / (total(reform(wch_cor[wmin,wch])*wch_step)/band_factor) $
		...
		if (use_norm_save eq 0) then norm_photo = flux_ref / (total(reform(wch_photo[wmin,wch])*wch_step)/band_factor) $
		...
		model_cor_ref = reform(wch_cor[wmin,*]) * norm_avg
		model_photo_ref = reform(wch_photo[wmin,*]) * norm_avg
		...
		for k=0L, num_temp-1 do begin
			model2[k,wnew] = model2_af * reform(wch_photo[k,wnew]) - $
				((model2_af-1.)/(CF-1.)) * (CF * reform(wch_photo[k,wnew]) - reform(wch_cor[k,wnew]))
		endfor
	endif else begin
		if keyword_set(debug) then print, 'WARNING: All (Si) abundance not fit as flux is low: ', flux_ref
	endelse
endif
...
if (parameters.logT ge 6.2) and (e_min lt 3.3) and (e_max gt 4.0) then begin
	...
	if (flux_ref gt (FLUX_MIN*4)) then begin
		...
		if keyword_set(debug) then print, 'WARNING: Ca abundance not fit as flux is low: ', flux_ref
	endelse
endif
...
if (parameters.logT ge 6.6) and (e_min lt 6.0) and (e_max gt 7.0) then begin
	...
	if (flux_ref gt (FLUX_MIN*2.)) then begin
		...
		if keyword_set(debug) then print, 'WARNING: Fe abundance not fit as flux is low: ', flux_ref
	endelse
endif
