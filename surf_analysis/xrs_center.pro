...
;
; CALLING SEQUENCE:
;	xrs_center, channel, [ filename, surffile, itime=itime, /debug, data=data, surfdata=surfdata]
;
; INPUTS:
...
;
;	NOTE:  use rxrs_center.pro if you want to do center analysis for 'X123', 'SPS', 'PSn' channels
;	(March 2018)
;+

pro xrs_center, channel, filename, surffile, itime=itime, debug=debug, $
						rocket=rocket, etu=etu, $
...
if n_params() lt 1 then begin
  print, 'Usage:  xrs_center, channel, [ filename, surffile, itime=itime, /debug, data=data, surfdata=surfdata]'
  return
endif
...
;			/rocket option for Rocket DataView files:  plotxrs.pro
...
if keyword_set(rocket) then begin
 ...
 if keyword_set(debug) then begin
  if (doSURF ne 0) then begin
    plot_rxrs_hydra, ch, filename, surffile, itime=integtime, data=data, surfdata=surfdata, /debug
  endif else begin
    plot_rxrs_hydra, ch, filename, itime=integtime, data=data, /debug
  endelse
 endif else begin
  if (doSURF ne 0) then begin
    plot_rxrs_hydra, ch, filename, surffile, itime=integtime, data=data, surfdata=surfdata
  endif else begin
    plot_rxrs_hydra, ch, filename, itime=integtime, data=data
  endelse
 endelse
endelse
...
if (ans eq 'Y') then begin
  ...
  if (numfov lt 2) then begin
    print, 'ERROR xrs_center: user did not select valid time range'
    return
  endif
  fovdata = data[wfov]
endif else begin
...
;
;	Debug time offsets seen between DataView and SURFER
;
