...
;
;	same as current.pro BUT uses make_mewe_spectrum.pro to get solar min and max Mewe spectra 
;
pro current_mewe, w, s, serr, wout, sigout, nolabel=nolabel, stopit=stopit, shortwave=shortwave

if n_params(0) lt 2 then begin
	print, 'Usage:  current_mewe, w, s, [/nolabel]'
	diode,w,s
endif

common euvflux4, sunlow, sunhigh

if n_elements(sunlow) le 1 then begin
  print, 'Making Mewe reference solar spectra...'
  sunlow=make_mewe_spectrum( [0.5, 1, 2], [0., 0., 1.0], /min )
  sunlow[1,*] = sunlow[1,*] * 1E-9  ; make flux 1E9 ph/s/cm^2
  sunhigh=make_mewe_spectrum( [0.5, 1, 2, 4], [0., 0, 1.0, 0.5], /max )
  sunhigh[1,*] = sunhigh[1,*] * 1E-9  ; make flux 1E9 ph/s/cm^2
...
plot, wvhigh, chigh, xrange=xrange, xs=1
