...
;
;	same as current.pro BUT uses Woods and Rottman [2002] 
;	reference solar spectra instead of EUV81 results
;
pro current_ref, w, s, serr, wout, sigout, nolabel=nolabel, stopit=stopit

if n_params(0) lt 2 then begin
	print, 'Usage:  current_ref, w, s, [/nolabel]'
	diode,w,s
endif

common euvflux2, sunlow, sunhigh
...
if n_elements(sunlow) le 1 then begin
  ...
  temp=read_dat(sdir+'ref_min_27day_11yr.dat')
  temp[0,*] = temp[0,*] * 10.	; convert nm to Angstroms
  temp[1,*] = temp[1,*] * 1E-9  ; make flux 1E9 ph/s/cm^2
  sunlow = temp[0:1,*]		; solar min.
  sunhigh = temp[0:1,*]		; solar max
  sunhigh[1,*] = sunhigh[1,*] * temp[3,*]
  ; sunlow = euv81( 90, 90, 2, adjust=1 )		; w = Angstroms
  ; sunhigh = euv81( 200, 200, 2, adjust=1 )	; flux = 10^9 photons/cm^2/s
  ; 
  ; now convert 1 nm spectra into 0.1 nm spectra
  ;
  temp1 = sunlow
  nwave = (n_elements(temp1)/2)*10
  sunlow=fltarr(2,nwave)
  sunlow[0,*] = findgen(nwave)+1.5
  sunlow[1,*] = (interpol( temp1[1,*], temp1[0,*], sunlow[0,*] ) ) > 0.0
  temp2 = sunhigh
  nwave = (n_elements(temp2)/2)*10
  sunhigh=fltarr(2,nwave)
  sunhigh[0,*] = findgen(nwave)+1.5
  sunhigh[1,*] = (interpol( temp2[1,*], temp2[0,*], sunhigh[0,*] ) ) > 0.0
  ; stop, 'Check out sunlow, sunhigh, temp1, temp2 ...'
