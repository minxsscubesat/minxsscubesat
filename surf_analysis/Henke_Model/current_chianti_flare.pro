...
;	same as current_chianti.pro but uses FLARE for solar cycle max
;
pro current_chianti_flare, w, s, serr, wout, sigout, nolabel=nolabel, stopit=stopit, shortwave=shortwave
...
common euvflux5f, sunlow, sunhigh
...
if n_elements(sunlow) le 1 then begin
  ...
  ;
  ;  solar max = solar min + 1% Flare  = (chsp[0,*] + chsp[2,*])/2. + 0.010 * chsp[3,*]
  ;
  sunhigh=dblarr(2,n_elements(chwave))
  sunhigh[0,*] = chwave
  sunhigh[1,*] = (chsp[0,*]+chsp[2,*])/2. + 0.010 * chsp[3,*]
...
for k=0L,kmax,kstep do begin
  ...
  if wgd(0) ne -1 then begin
    ...
    if (plow gt limit) or (phigh gt limit) or (k eq 0) then begin
