...
;
;	same as current.pro BUT uses NRLEUV Flare Spectra Version 2 (2004 QS spectrum)
;	instead of EUV81 results
;
pro current_nrleuv2, w, s, serr, wout, sigout, nolabel=nolabel, stopit=stopit

if n_params(0) lt 2 then begin
	print, 'Usage:  current_nrleuv2, w, s, [/nolabel]'
	diode,w,s
endif
...
if n_elements(sunlow) le 1 then begin
  ...
  temp2=read_dat(sdir+'NRLEUV_sp_v2.dat')    ;  or  'nrleuv_bastille_flare.dat'
  temp = dblarr(3,n_elements(temp2[0,*]))
  tfactor = 1.E-9 ;  already in photons/s/cm^2   W->ph == 5.03556E11 * temp2[0,*]
  temp[0,*] = temp2[0,*] 	; wavelength in Angstroms
  temp[1,*] = temp2[1,*] * tfactor   ; make flux 1E9 ph/s/cm^2
  temp[2,*] = temp2[1,*] * 2. * tfactor  ; Max is 2 x Min
