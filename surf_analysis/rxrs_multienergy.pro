...
;	Updated in 2018 for Rocket XRS with MinXSS-like packets (plot_rxrs_hydra.pro)
;
; CATEGORY:
;	SURF calibration procedure
;
; CALLING SEQUENCE:
;	rxrs_multienergy, channel [ , /debug, data=data]
;
; INPUTS:
;	channel		Channel values for XRS can be  'A1', 'A2', 'B1', 'B2'
...
;	3/28/18		Tom Woods  Updated for Rocket XRS-X123-SPS
;
;+

pro rxrs_multienergy, channel, debug=debug, data=data
;
;	1.  Check input parameters
;
data = -1L
if n_params() lt 1 then begin
  print, 'Usage:  xrs_multienergy, channel [ , /debug, data=data ]'
  return
endif
...
; not needed in 2018 version as integration time is in SCI packets
...
for k=0, nenergy-1 do begin
  ...
  plot_rxrs_hydra, ch, dfile, sfile, data=d, debug=debug
