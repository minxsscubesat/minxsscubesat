...
;
if (rnum eq 36.233) or (rnum eq 36.240) or (rnum eq 36.286) or (rnum eq 36.290) or (rnum eq 36.300) or (rnum eq 36.318) then begin
  wordmask = '03FF'X
  sync1value = '03EB'X
  sync1offset = 0L
...
for k=kstart,kend do begin
  ...
  if ((data[sync1offset] and wordmask) eq sync1value) and $
    ...
    if keyword_set(analog) then begin
      ...
      if (acnt ge 0) and (acnt lt 10) and keyword_set(debug) then begin
        print, 'Analog Record'
        help, atemp, /struct
        print, ' '
