...
ncol = 85L ; JPM: this should be 82L because no extra time information included 
nrow = 3L
...
; JPM: May not even require syncwords, but could still check
wordmask = 'FFFF'X		; don't need to mask 
sync1value = '2840'X ; JPM: 36.300 is 2840 - confirmed
sync1offset = 0L ; JPM: change to nint - 1L

sync2value = 'FE6B'X ; JPM: 36.300 mask is FE6B - confirmed
sync2offset = nint-1L ; JPM: change to nint-2L
...
;
;	find first valid time ; JPM: this isn't there in real time
;
...
if keyword_set(classic) then begin
  ...
  coferr = 0L
  cxy = [6,0,1,1,12,0] ; JPM: Change 6 to 2 for XRI
  ;ctcnt = 0L
  ;ctemp = cwords
  ;ctmax = n_elements(ctemp)
endif

; JPM: Make common block with ping-pong buffer for both images (one to be filled, one that is most recently finished)
if keyword_set(amegs) then begin 
  ...
  aacnt = 0L ; JPM: How many full images written to file
  mfidvalue1 = 'FFFF'X
  mfidvalue2 = 'AAAA'X
  awcnt = 0L ; JPM: How many have been read
  axy = [18,0,2,1,33,0] ; JPM: Change 18 to 14
endif
if keyword_set(bmegs) then begin
  ...
  mfidvalue2 = 'AAAA'X
  bwcnt = 0L
  bxy = [19,0,2,1,33,0] ; JPM: Change 19 to 15
endif

; JPM: Shove all TCP/IP data into a log file on disk as backup
...
;
; JPM: This whole loop should just sort the data using the extract_item
