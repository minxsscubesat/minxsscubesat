...
;	This is 2018 version that uses MinXSS FSW and HYDRA interface with XRS-X123-SPS channels.
;
; USAGE
;   read_hydra_xrs, input, hk=hk, sci=sci, log=log, sps=sps, /hexdump, /verbose
;
; INPUTS:
...
;	sps [structure];		  Return array of picoSIM-SPS science packets
;
; OPTIONAL OUTPUTS:
;   None
;
...
;	2)	Updated 3/24/2018 to include new definitions for PicoSIM-SPS channels
;
;+
...
junk = temporary(hexdump)
  ...
  if (n_params() lt 1) then begin
    print, 'USAGE: read_hydra_xrs, input_file, hk=hk, sci=sci, log=log, sps=sps, $'
    print, '              /hexdump, /verbose'
    input=' '
  endif
  IF size(input[0], /TYPE) EQ 7 THEN if (strlen(input) lt 2) then begin
    ; find file to read
    input = dialog_pickfile(/read, title='Select HYDRA telemetry file to read', filter='*')
  ...
  PACKET_ID_LOG = 29
  PACKET_ID_SCI = 44
  PACKET_ID_PICOSIM = 50	; NEW 2018 for PicoSIM-SPS packets

  PACKET_ID_DIAG = 35		; NOT used for Rocket XRS
  PACKET_ID_IMAGE = 42
  PACKET_ID_SPS = 43		; NOT used for Rocket XRS (used for MinXSS ADCS operations)
  PACKET_ID_ADCS1 = 38
  PACKET_ID_ADCS2 = 39
  PACKET_ID_ADCS3 = 40
  PACKET_ID_ADCS4 = 41
  ...
  PACKET_ID_PICOSIM_PLAYBACK = PACKET_ID_PICOSIM + '40'X
  ...
  ;
  ; PicoSIM-SPS (SPS) Packet Structure definition
  ;
  sps_count = 0L
  PICOSIM_VIS_NIR_CHANNELS = 6
  PICOSIM_SPS_CHANNELS = 4
  sps_struct1 = { apid: 0, seq_flag: 0, seq_count: 0, data_length: 0L, time: 0.0D0, $
    	Last_CMD_Letter: 0, Last_CMD_Flag: 0, Record_Count: 0L, $
    	Year: 0, Month: 0, Day: 0, Hour: 0, Minute: 0, Second: 0, jd: 0.0D0, $
 		VIS_NIR_Type: 0, picoSIM_Temp_C: 0.0, EPS_Board_Temp_C: 0.0, $
 		VIS_NIR_Num_Avg: 0, VIS_NIR_Signal: fltarr(PICOSIM_VIS_NIR_CHANNELS), VIS_NIR_Temp_C: 0.0, $
   		SPS_Num_Avg: 0, SPS_QD_Signal: fltarr(PICOSIM_SPS_CHANNELS), $
    	SPS_QD_Sum: 0.0, SPS_QD_X: 0.0, SPS_QD_Y: 0.0, $
    	checkbytes: 0L, SyncWord: 0.0}
  ...
  while (index lt (inputSize-1)) do begin
    if (data[index-1] eq SYNC_BYTE1) and (data[index] eq SYNC_BYTE2) then begin
      ...
      if (packet_id eq PACKET_ID_HK) then begin
      ...
      endif else if (packet_id eq PACKET_ID_PICOSIM) then begin
        ;
        ;  ************************
        ;  PicoSIM-SPS Packet (if user asked for it)
        ;  ************************
        ;
        if arg_present(sps) then begin
          sps_struct1.apid = packet_id_full  ; keep Playback bit in structure
          sps_struct1.seq_flag = ishft(long(data[pindex+2] AND 'C0'X),-6)
          sps_struct1.seq_count = packet_seq_count
          sps_struct1.data_length = packet_length
          sps_struct1.time = packet_time
          sps_struct1.checkbytes = long(data[pindex+92]) + ishft(long(data[pindex+93]),8)
          sps_struct1.SyncWord = (long(data[pindex+94]) + ishft(long(data[pindex+95]),8))


    	  sps_struct1.Last_CMD_Letter = fix(data[pindex+12])
    	  sps_struct1.Last_CMD_Flag = fix(data[pindex+13])
    	  sps_struct1.Record_Count = long(data[pindex+14]) + ishft(long(data[pindex+15]),8)
    	  sps_struct1.Year = fix(data[pindex+16]) + ishft(fix(data[pindex+17]),8)
    	  sps_struct1.Month = fix(data[pindex+18])
    	  sps_struct1.Day = fix(data[pindex+19])
    	  sps_struct1.Hour = fix(data[pindex+20])
    	  sps_struct1.Minute = fix(data[pindex+21])
    	  sps_struct1.Second = fix(data[pindex+22])
    	  sps_struct1.jd = ymd2jd( sps_struct1.Year, sps_struct1.Month, sps_struct1.Day + $
    	  			(sps_struct1.Hour + sps_struct1.Minute/60. + sps_struct1.Second/3600.)/24.D0 )

 		  sps_struct1.VIS_NIR_Type = fix(data[pindex+23])
 		  ; IDL float() will convert byte array to float if use second parameter as 0
 		  ; If numbers come out odd, then reverse the byte array
		  sps_struct1.picoSIM_Temp_C = float( data[pindex+24:pindex+27], 0 )
		  sps_struct1.EPS_Board_Temp_C = float( data[pindex+28:pindex+31], 0 )
		  sps_struct1.VIS_NIR_Num_Avg = fix(data[pindex+32]) + ishft(fix(data[pindex+33]),8)
		  for ii=0, PICOSIM_VIS_NIR_CHANNELS-1 do begin
		  	sps_struct1.VIS_NIR_Signal[ii] = float( data[pindex+34+ii*4:pindex+37+ii*4], 0 )
		  endfor
		  sps_struct1.VIS_NIR_Temp_C = float( data[pindex+58:pindex+61], 0 )

		  sps_struct1.SPS_Num_Avg = fix(data[pindex+62]) + ishft(fix(data[pindex+63]),8)
		  for ii=0, PICOSIM_SPS_CHANNELS-1 do begin
			  sps_struct1.SPS_QD_Signal[ii] = float( data[pindex+64+ii*4:pindex+67+ii*4], 0 )
		  endfor
		  sps_struct1.SPS_QD_Sum = float( data[pindex+80:pindex+83], 0 )
		  sps_struct1.SPS_QD_X = float( data[pindex+84:pindex+87], 0 )
		  sps_struct1.SPS_QD_Y = float( data[pindex+88:pindex+91], 0 )

          if (sps_count eq 0) then sps = replicate(sps_struct1, N_CHUNKS) else $
            if sps_count ge n_elements(sps) then sps = [sps, replicate(sps_struct1, N_CHUNKS)]
          sps[sps_count] = sps_struct1
          sps_count += 1
        endif
        ...
        if arg_present(sci) then begin
          ...
          if ((sci_struct1.seq_flag AND '01'X) eq 1) then begin
            ...
            for ii=0,X123_FIRST_LENGTH-1 do sci_raw_data[ii] = data[pindex+106+ii]  ; old offset was 82
  ...
  if arg_present(sps) and n_elements(sps) ne 0 then sps = sps[0:sps_count-1]
...
;  endif
  ...
  if keyword_set(verbose) then begin
    ...
    if (sps_count gt 0) then print, 'Number of PS-SPS Packets = ', sps_count
  endif
