...
while (not eof(lun)) do begin
	...
	if (nstr ge 3) then begin
		...
		if (darray[0] eq 'DS.CDHA') then begin
			...
			if (theCDHversion le 0) then theCDHversion = data[data_num].cdh_version
			if (theCDHversion ge 3.05) then begin
				; also read Instrument Name and command info
				data[data_num].cdh_instr_name = darray[7]
				data[data_num].cdh_command_info = darray[8]
				data[data_num].cdh_command_index = fix(darray[10])
			endif
		endif
		...
		if (darray[0] eq 'DS.ASPS') then begin
			...
			if (darray[1] eq '1') then begin
				data[data_num].sps1_error_count = fix(darray[2])
				data[data_num].sps1_power_mon = float(darray[3])
				data[data_num].sps1_ground_mon = float(darray[4])
				data[data_num].sps1_temperature = float(darray[5])
			endif else begin
				; it thus be SPS-2
				data[data_num].sps2_error_count = fix(darray[2])
				data[data_num].sps2_power_mon = float(darray[3])
				data[data_num].sps2_ground_mon = float(darray[4])
				data[data_num].sps2_temperature = float(darray[5])
			endelse
			if (darray[6] eq '2') then begin
				data[data_num].sps2_error_count = fix(darray[7])
				data[data_num].sps2_power_mon = float(darray[8])
				data[data_num].sps2_ground_mon = float(darray[9])
				data[data_num].sps2_temperature = float(darray[10])
		...
		if (darray[0] eq 'DS.1SPS') then begin
			...
			data[data_num].sps1_loop_count = fix(darray[1])
			for i=0,3 do data[data_num].sps1_data[i] = float(darray[i+2])
			data[data_num].sps1_quad_sum = double(darray[6])
			data[data_num].sps1_quad_x = float(darray[7])
			data[data_num].sps1_quad_y = float(darray[8])
		endif
		if (darray[0] eq 'DS.2SPS') then begin
			; parse DS.2SPS data line
			data[data_num].have_sps2 = 1
			data[data_num].sps2_loop_count = fix(darray[1])
			for i=0,3 do data[data_num].sps2_data[i] = float(darray[i+2])
			data[data_num].sps2_quad_sum = double(darray[6])
			data[data_num].sps2_quad_x = float(darray[7])
			data[data_num].sps2_quad_y = float(darray[8])
		endif
