...
;
; CALLING SEQUENCE:
;	dualsps_plot_ccsds, filename, [ channel, surffile, /debug, data=data, surfdata=surfdata, plotdata=plotdata]
;
; INPUTS:
;	filename	HYDRA telemetry data file from Dual-SPS instrument
;	channel		Channel names can be 'SPS', 'SPS_X', 'SPS_Y', 'SPS_1' ... 'SPS_4'
;					(this is for single SPS channel as for Magnetograph TTM)
...
pro dualsps_plot_ccsds, filename, channel, surffile, debug=debug, $
...
if n_params() lt 1 then begin
  print, 'Usage:  dualsps_plot_ccsds, filename, [ channel, surffile, /debug, data=data, surfdata=surfdata, plotdata=plotdata]'
  return
endif
...
if n_params() lt 2 then channel = 'SPS'  ; default value for Magnetograph TTM
...
then begin
  print, 'ERROR dualsps_plot_ccsds: Invalid Channel name.  Expected SPSn, SPSn_X, SPSn_Y, SPSn_1-SPSn_4 for n=1 or 2.'
  return
endif
...
;
data = dualsps_read_file_ccsds( filename, /verbose )  ;  add messages=msg to call for Messages
if (n_elements(data) lt 2) then begin
  print, 'ERROR dualsps_plot_ccsds: No valid data found for ', filename
  return
endif
...
; Non-CCSDS packet is different
; sod = data.hour * 3600.D0 + data.minute * 60. + data.second + utoffset
; regular spacing of time assumed
; sod = findgen(n_elements(data)) * (data[0].tlm_integ_time/1000.)
; CCSDS time is GPS seconds and JD
sod = (data.jd - (long(median(data.jd)) )) *24.*3600. + utoffset
plotdata.time = sod
...
if (ch eq 'SPS') then begin
  ...
  ch_sps_data = data.sps_diode_data
  ...
  sdark = 16450.	; quad diode dark signal
  darkFactor = 1.00
  plotdata.signal = data.SPS_data[0]
  ch_sps_data = data.sps_diode_data
  ...
  sdark = 16450.	; quad diode dark signal
  darkFactor = 1.00
  plotdata.signal = data.SPS_data[1]
  ch_sps_data = data.sps_diode_data
  ...
  sdark = 16450.	; quad diode dark signal
  darkFactor = 1.00
  plotdata.signal = data.SPS_data[2]
  ch_sps_data = data.sps_diode_data
  ...
  sdark = 16450.	; quad diode dark signal
  darkFactor = 1.00
  plotdata.signal = data.SPS_data[3]
  ch_sps_data = data.sps_diode_data
endif

num_data = n_elements(plotdata)

; reduce plotdata to the GOOD data = positive time
wgood = where( sod gt 0, numgood )
if (numgood lt 2) then begin
	print, 'WARNING dualsps_plot_ccsds: Time Offsets found.'
	wgood = indgen(num_data)
	; return
endif else begin
	print, 'dualsps_plot_ccsds: reduced SPS data set to ', strtrim(numgood,2), ' elements.'
...
if (isQuad ne 0) then begin
  ...
  for j=0,3 do begin
    ...
    quadsignal[j,*] = ch_sps_data[j,*] - sdark * fscale
  endfor
  ...
  if (ch eq 'SPS') then begin
  	...
  	plotdata.quadx = data[wgood].SPS_quad_x
  	plotdata.quady = data[wgood].SPS_quad_y
  	doPrint = 1
  endif
endif
...
if (doSURF ne 0) then begin
  ...
  if keyword_set(debug) then begin
    print, ' '
    print, 'dualsps_plot_ccsds: First HYDRA time = ', strtrim(plotdata[0].time,2)
    print, 'dualsps_plot_ccsds: First SURF  time = ', strtrim(surfdata[0].time_sod,2)
    extra_offset = 0.0
...
plot, ptime, psignal, xmargin=xm, ymargin=ym, ystyle=1, $
...
if keyword_set(debug) then begin
  ; if (debug ge 1) then $
  stop, 'STOP: DEBUG at end of dualsps_plot_ccsds.pro...'
endif
